...
def recommend(genre):
    ...
    df_filter = df_genre[['name','year','rating','genre','certificate','run_time','budget' ,'box_office','profit','directors']]
    ...
    def create_p_rating():
        ...
        p = figure(title="Your genre sorted according to IMDB Rating ", x_axis_label='IMDB RATING',
                   y_axis_label='YEAR RELEASED', tools=[TOOLTIPS])
    ...
    def create_p_collection():
        TOOLTIPS_NEW = HoverTool(tooltips=[
            ("Rank", "$index"),
            ("Movie Name", "@name"),
            ("Budget", "@budget"),
        ...
        p_collection = figure(width=1000, height=1000, title="Your genre's Box office collection and Budget",
                       x_axis_label='Budget', y_axis_label='Box Office Collection', tools=[TOOLTIPS_NEW])
        p_collection.xaxis[0].formatter = NumeralTickFormatter(format='0')
        p_collection.yaxis[0].formatter = NumeralTickFormatter(format='0')
        circle_new = p_collection.circle('budget', 'box_office', fill_color="maroon", size=10, source=df_filter)
    ...
    def cert_rating():
        ...
        TOOLTIPS_CERT = HoverTool(tooltips=[
            ("Movie Name", "@name"),
            ("certificate", "@certificate"),
            ("rating", "@rating")
        ])

        p_cert = figure(x_range=cert.unique(), height=700, width = 800, x_axis_label = 'CERTIFICATE', y_axis_label = 'IMDB RATING', title="Rating vs Certification of Movies based on your selected Genre",
                   toolbar_location=None, tools=[TOOLTIPS_CERT])
        p_cert.vbar(x='certificate', top='rating', width=0.5, fill_color = "green", source = df_filter)
    ...
    def cert_profit():
        ...
        TOOLTIPS_PROFIT = HoverTool(tooltips=[
            ("Movie Name", "@name"),
            ("Genre", "@genre"),
            ("Profit", "@profit"),
            ("IMDB Rating", "@rating"),
        ...
        p_profit = figure(x_range=name.unique(), height=800, width = 4000, x_axis_label = 'MOVIE NAME', y_axis_label = 'PROFIT', title="Movie profit according to your genre",
    ...
    def cert_director():
        ...
        TOOLTIPS_DIRECTOR = HoverTool(tooltips=[
            ("Movie Name", "@name"),
            ("Profit", "@profit"),
            ("Directors", "@directors")
        ])

        p_director = figure(x_range=name.unique(), height=800, width = 4000, x_axis_label = 'DIRECTOR NAME', y_axis_label = 'PROFIT', title="List of Directors having movies with highest profits",
    ...
    def cert_director_rating():
        ...
        TOOLTIPS_DIRECTOR_RATING = HoverTool(tooltips=[
            ("Movie Name", "@name"),
            ("IMDB Rating", "@rating"),
            ("Directors", "@directors")
        ])

        p_director_rating = figure(x_range=name.unique(), height=800, width = 4000, x_axis_label = 'DIRECTOR NAME', y_axis_label = 'IMDB RATING', title="List of Directors having movies with highest IMDB Ratings",
...
movies = ['Drama','drama','Crime','crime','Action','action','Biography','biography','History','history','Adventure','adventure','Western','western','Romance','romance','Sci-fi','sci-fi','Mystery','mystery','Family','family','Fantasy','fantasy','Thriller','thriller','War','war','Animation','animation','Music','music','Horror','horror','Comedy','comedy','Film-Noir','film-noir']

print('''Welcome to Movie Recommendation System.

This system shows you the various details and Visualizations relating to IMDB Rating, Movie Release Year, Profit and more.
